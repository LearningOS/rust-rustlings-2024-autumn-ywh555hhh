    ...
    T: PartialOrd,
{
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        macro_rules! consume_node {
            ($node: expr,$ptr: expr, $target: expr) => {
        ...
        while let (Some(mut node_a), Some(mut node_b)) = (ptr_a, ptr_b) {
            unsafe {
                if (*node_a.as_ptr()).val <= (*node_b.as_ptr()).val {
                    consume_node!(node_a, ptr_a, result);
                } else {
                    consume_node!(node_b, ptr_b, result);
                }
            }
        }
    ...
    unsafe fn append_node(&mut self, node: NonNull<Node<T>>) {
